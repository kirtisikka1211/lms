...
def leave_request(request):
    if request.method == 'POST':
        ...
        try:
            ...
            print(start_list)
            if int(start_list[1])==int(end_list[1]):
                if int(start_list[2])< int(end_list[2]):
                    msg.send()
                    messages.success(request, 'Leave request submitted successfully.')  
                else:  
                    messages.error(request, 'Please mention a valid time period.')
            elif int(start_list[1])< int(end_list[1]):
                msg.send()
                messages.success(request, 'Leave request submitted successfully.')
            elif int(start_list[0])< int(end_list[0]):
                msg.send()
                messages.success(request, 'Leave request submitted successfully.')
